#!/bin/bash
# Quick update script - copy this to server and run

echo "🔄 Updating to webclip-installer..."

# Stop service
sudo systemctl stop lara-ota

# Backup
sudo cp /opt/lara-ota/server.py /opt/lara-ota/server.py.old

# Update server.py
sudo tee /opt/lara-ota/server.py > /dev/null << 'ENDOFFILE'
#!/usr/bin/env python3
...
echo ""
echo "✅ Updated to webclip-installer!"
echo "🌐 Test: https://liver-vintage-marco-thus.trycloudflare.com"
